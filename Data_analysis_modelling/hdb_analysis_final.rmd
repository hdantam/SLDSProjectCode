...
```{r}
ggplot(resale, aes(x = reorder(town, resale_price, median), 
                   y = resale_price)) +
  geom_boxplot(fill = "#4C72B0", alpha = 0.7, outlier.alpha = 0.5) +
  labs(
    title = "HDB Resale Price by Town (Ordered by Median Price)",
    x = "Town",
    y = "Resale Price (SGD)"
  ) +
  theme_minimal(base_size = 14) +
